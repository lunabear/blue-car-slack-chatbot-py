...
from constants import S3_BUCKET_NAME, DEFAULT_REGION
...
def get_object_contents(object_key, region=DEFAULT_REGION, bucket_name=get_bucket_name()):
    print(
        f'region:{region}, bucket_name:{bucket_name}, source_key:{object_key}')
    try:
        ...
        print(f'put_object_contents: {e}')
        raise e


def download_object(object_key, filename, region=DEFAULT_REGION, bucket_name=get_bucket_name()):
    print(
        f'region:{region}, bucket_name:{bucket_name}, key:{object_key}, filename:{filename}'
    )
    try:
        s3_client = _find_client_from_cache(region)
        s3_client.download_file(Bucket=bucket_name, Key=object_key, Filename=filename)
    except Exception as e:
        raise e
...
def put_object_contents(source_contents, object_key, region=DEFAULT_REGION, bucket_name=get_bucket_name()):
    ...
    """
    print(
        f'region:{region}, bucket_name:{bucket_name}, destination_key:{object_key}')
    try:
        s3_client = _find_client_from_cache(region)
...
def delete_object(object_key, region=DEFAULT_REGION, bucket_name=get_bucket_name()):
    ...
    """
    print(
        f'delete object, region:{region}, bucket_name:{bucket_name}, object_key:{object_key}')
    try:
        s3_client = _find_client_from_cache(region)
        response = s3_client.delete_object(Bucket=bucket_name, Key=object_key)
        return response
    except Exception as e:
        print(f'delete_object: {e}')
        raise e
...
def delete_all_objects_with_prefix(prefix, region=DEFAULT_REGION, bucket_name=get_bucket_name()):
    ...
    try:
        ...
        print(f'delete_all_objects_with_prefix: {e}')
        raise e
...
def copy_all_object_same_region(source_prefix, destination_prefix,
    ...
    try:
        ...
        for content in response['Contents']:
            ...
            print(f's_key:{source_key}, d_key:{destination_key}')
        ...
        print(f'copy_all_object_same_region: {e}')
        raise e


def list_objects_with_prefix(prefix, region=DEFAULT_REGION, bucket_name=get_bucket_name()):
    """ 버킷에서 해당 접두어를 가진 객체를 리스트로 반환한다.
    :param region: 버킷의 리전
    :param bucket_name: 버킷명
    :param prefix: 객체의 접두어
    """
    ...
    try:
        ...
        print(f'list_objects_with_prefix: {e}')
        raise e
...
def copy_object_same_region(source_key, destination_key, region=DEFAULT_REGION, bucket_name=get_bucket_name()):
    ...
    """
    print(
        f'copy object, region_name:{region} bucket_name:{bucket_name} source_key:{source_key} destination_key:{destination_key}')
    try:
        s3_client = _find_client_from_cache(region)
        ...
        print(f'copy_object_same_region: {e}')
        raise e
